 ...
 * the below create 'MyOppApp' app (module) and adds 'AngularForce', 'AngularForceObject' and
 * 'Opportunity' modules as its dependencies
 *
 * PS: This function is called when jquery.ready is triggered in index.html
 */
function bootStrapAngular() {
    var angularApp = angular.module('MyOppApp', ['AngularForce', 'AngularForceObject', 'Opportunity']);
 ...
 */
angular.module('Opportunity', []).factory('Opportunity', function (AngularForceObject) {
    return AngularForceObject({type: 'Opportunity', fields: ['Name', 'ExpectedRevenue', 'StageName', 'CloseDate', 'Id'], where: 'WHERE IsWon = TRUE'});
});
 ...
 * @param Opportunity Salesforce Object described as AngularJS Module that returns AngularForceObject
 * @param $location  AngularJS Location service - Used to change JQM page
 * @constructor
 */
function ListCtrl($scope, Opportunity, $location) {
